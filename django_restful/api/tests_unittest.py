...
"""
    对api模块进行接口测试
    编写测试用例基本规则:
    (1)在强调一遍: 测试类中的测试用例必须以 'test' 开头 要不然unittest模块无法检测到
    (2)在新建用户并且为其分组时，url要带全，是'xxxgroups/' 而不是'xxxgroups' 一个会重定向 一个不会重定向
    (3)在未设置 setUp() 和 tearDown() 方法前，数据之间肯定会相互干扰，需要用到 unittest的跳过测试装饰器 @unittest.skip('reason')
...
@unittest.skip('skip this class to test GroupTest')
class UserTest(unittest.TestCase):
    ...
    @unittest.skip('just skip for other test...')
    ...
    @unittest.skip('skipping for other method...')
